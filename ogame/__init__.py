...
class OGame(object):
    ...
    def buy_offer_of_the_day(self):
        response = self.session.get(url=self.index_php + 'page=ingame&component=traderOverview').text
        response2 = self.session.post(
            url=self.index_php + 'page=ajax&component=traderimportexport',
            data={'show': 'importexport',
                  'ajax': 1},
        ...
        try:
            ...
            gc.collect()
            return f' Already bought item!'
        except Exception as e:
            locals()["err"] = e
        try:
            item_price = bs4.find_partial(class_='price js_import_price').text
            item_price = int(item_price.replace('.', '').replace(',', ''))
        except Exception as e:
            gc.collect()
            return f'err: {e}, Failed to extract price!'
        ...
        try:
            ...
            gc.collect()
            return f'err: {e}, Failed to extract resources!'

        try:
            import_token = re.search(r'var token\s?=\s?"([^"]*)";', response2).group(1)
        except Exception as e:
            gc.collect()
            return f'err: {e}, Failed to extract token!'
        ...
        try:
            ...
            gc.collect()
            return f'err: {e}, Failed to extract multiplier!'
        ...
        response3 = self.session.post(
            url=self.index_php + 'page=ajax&component=traderimportexport&ajax=1&action=trade&asJson=1',
            data=form_data,
        ...
        try:
            ...
            gc.collect()
            return f'err: {e}, Failed to extract newAjaxToken!'

        form_data2 = {'action': 'takeItem',
                      'token': '{}'.format(new_token),
                      'ajax': '1'}

        response4 = self.session.post(
            url=self.index_php + 'page=ajax&component=traderimportexport&ajax=1&action=takeItem&asJson=1',
            data=form_data2,
            headers={'X-Requested-With': 'XMLHttpRequest'}).json()
